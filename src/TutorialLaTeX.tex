...
\makeindex[columns=3, title=Índice, intoc] % constrói o índice.
...
O arquivo do código-fonte em \LaTeX\ deve conter apenas bytes que representem caracteres, sem nenhuma informação adicional. Trata-se do denominado arquivo de texto plano (txt). No sistema \LaTeX, este arquivo recebe a extensão .tex\index{tex@\texttt{\small{.tex}}}. A codificação recomendada para este arquivo é a codificação UTF-8 ou Latin1, dependendo do sistema operacional que está processando o \LaTeX.\index{latex@\texttt{\small{LaTeX}}}

Pode-se utilizar qualquer editor de texto puro para produzir o arquivo .tex, entretanto, é recomendável utilizar um editor \LaTeX\ para obter uma melhor produtividade. Basicamente, precisa estar instalado no seu computador: uma distribuição \TeX, por exemplo, TeX Live\index{tex live@\texttt{\small{Tex Live}}} ou MikTex\index{miktex@\texttt{\small{MikTex}}}; e um editor \LaTeX, por exemplo, TeXstudio\index{texstudio@\texttt{\small{TeXstudio}}}, Texmaker\index{texmaker@\texttt{\small{Texmaker}}} ou TeXworks\index{texworks@\texttt{\small{TeXworks}}}.

Mas, atente-se, editar um documento em \LaTeX\ não será WYSIWYG (\textit{What You See Is What You Get})\index{wysiwyg@\texttt{\small{WYSIWYG}}}. Será, literalmente, editar o código-fonte do documento, inserindo os comandos de formatação do texto, entre seu conteúdo.
...
A área entre \texttt{\textbackslash documentclass\{...\}} e \texttt{\textbackslash begin\{document\}} é denominada preâmbulo.\index{preambulo@\texttt{\small{preâmbulo}}} Neste preâmbulo, ficam os comando que afetam todo o documento. Como as chamadas do uso de pacotes, as definições de parâmetros de comandos, criação de novos comandos etc. A área entre \texttt{\textbackslash begin\{document\}} e \texttt{\textbackslash end\{document\}}\index{document@\texttt{\small{document}}}, após o preâmbulo, forma o bloco principal, o ambiente do documento, onde fica todo o conteúdo do documento.

\subsection*{Ambientes}
\addcontentsline{toc}{subsection}{Ambientes}

Na linguagem \LaTeX, um bloco é definido entre os caracteres \texttt{\{} e \texttt{\}} ou entre os comandos \texttt{\textbackslash begin\{\}}\index{begin@\texttt{\small\textbackslash begin}} e \texttt{\textbackslash end\{\}}\index{end@\texttt{\small\textbackslash end}}. Estes comandos formam um bloco de ambiente. Os demais comandos inseridos dentro de um bloco, ou de um ambiente, tem seu efeito restrito ao interior deste bloco, ou ambiente.
...
\index{\texttt{\small\textbackslash \#}}
\index{\texttt{\small\textbackslash \$}}
\index{\texttt{\small\textbackslash \%}}
\index{textasciicircum@\texttt{\small\textbackslash \textasciicircum}}
\index{\texttt{\small\textbackslash \&}}
\index{\texttt{\small\textbackslash \_}}
\index{\texttt{\small\textbackslash \{}}
\index{\texttt{\small\textbackslash \}}}
\index{\texttt{\small\textbackslash \~{}}}
\index{textasciitilde@\texttt{\small\textbackslash textasciitilde}}
\index{textbackslash@\texttt{\small\textbackslash textbackslash}}
...
\begin{multicols}{2}
É possível inserir comentários no código-fonte do arquivo \LaTeX. São informações que não serão processadas ou impressas. Os comentários de uma linha ficam após o caractere \texttt{\%}\index{\texttt{\small{\%}}}. Os comentários com mais de uma linha ficam em um bloco de ambiente \texttt{\{comment\}}\index{comment@\texttt{\small{comment}}}:
\columnbreak
\begin{small}
\begin{verbatim}
...
O \LaTeX\ inclui alguns comandos básicos, nativos, porém existem muitos outros comandos úteis que são implementados com o uso de pacotes, ativados pelo código-fonte do seu documento. Você poderá usar um pacote no seu documento em \LaTeX\ desde que tenha o respectivo pacote instalado em seu sistema \LaTeX. Assim, para usar e ativar um pacote, inclua o comando \texttt{\textbackslash usepackage\{nomedopacote\}}\index{usepackage@\texttt{\small\textbackslash usepackage}}. Além do mais, alguns pacotes podem receber parâmetros, por exemplo:
\begin{small}
\begin{verbatim}
...
Alguns comandos possuem duas versões, que são especificadas com a omissão ou o acréscimo do caractere \texttt{*}\index{\texttt{\small{*}}} (asterisco). Você verá\footnote{Sempre acompanhe, em paralelo, o arquivo .tex deste tutorial.}, no decorrer deste tutorial, estas duas versões. Nota: Muitos comandos nativos são originais do \TeX\ e, para simplificar, este tutorial generaliza com o nome \LaTeX.

O primeiro comando no código-fonte em \LaTeX\ é o comando \texttt{\textbackslash documentclass}\index{documentclass@\texttt{\small\textbackslash documentclass}}. Nele se define a classe do documento (ex. article\index{article@\texttt{\small{article}}}, report\index{report@\texttt{\small{report}}}, book\index{book@\texttt{\small{book}}}, letter\index{letter@\texttt{\small{letter}}}, beamer\index{beamer@\texttt{\small{beamer}}} ou memoir\index{memoir@\texttt{\small{memoir}}}) e os parâmetros para tamanho do papel e da fonte, lados de impressão etc.:
\begin{small}\index{a4paper@\texttt{\small{a4paper}}}
\begin{verbatim}
   \documentclass[a4paper,12pt,oneside]{book}
\end{verbatim}
\end{small}
...
A linguagem \LaTeX\ permite criar novos comandos, através do comando \texttt{\textbackslash newcommand}\index{newcommand@\texttt{\small\textbackslash newcommand}}. O novo comando recebe um nome e uma definição. Isto possibilita, por exemplo, criar um comando que retorne um valor, um texto qualquer:
\begin{small}
\begin{verbatim}
   \newcommand{\nome}{valor}
...
Os comandos existentes são redefinidos com o comando \texttt{\textbackslash renewcommand}\index{renewcommand@\texttt{\small\textbackslash renewcommand}}, exemplo:
\begin{small}\index{contentsname@\texttt{\small\textbackslash contentsname}}\index{thepage@\texttt{\small\textbackslash thepage}}
\begin{verbatim}
...
Ambientes também podem ser criados ou redefinidos, com os comandos \texttt{\textbackslash newenvironment}\index{newenvironment@\texttt{\small\textbackslash newenvironment}} e \texttt{\textbackslash renewenvironment}\index{renewenvironment@\texttt{\small\textbackslash renewenvironment}}, da mesma forma dos novos comandos. A sintaxe é semelhante, veja um exemplo:
\begin{multicols}{2}
\begin{small}
\begin{verbatim}
\newenvironment{moldura}
...
pt\index{pt@\texttt{\small{pt}}} & pontos (1/72 polegadas)\\
mm\index{mm@\texttt{\small{mm}}} & milímetros\\
cm\index{cm@\texttt{\small{cm}}} & centímetros\\
in\index{in@\texttt{\small{in}}} & polegadas\\
ex\index{ex@\texttt{\small{ex}}} & altura de um x minúsculo na fonte corrente\\
em\index{em@\texttt{\small{em}}} & largura de um M maiúsculo na fonte corrente\\
mu\index{mu@\texttt{\small{mu}}} & unidade matemática igual à 1/18em\\
\end{tabular}
...
Se o pacote foi carregado com a opção \texttt{[dvipsnames]}\index{dvipsnames@\texttt{\small{dvipsnames}}}, então um total de 68 cores estarão pré-definidas:
...
Para definir novas cores, insira os comandos no preâmbulo, seguindo estes exemplos:\index{definecolor@\texttt{\small\textbackslash definecolor}}
\begin{footnotesize}
\begin{verbatim}
...
\index{colorlet@\texttt{\small\textbackslash colorlet}}
\begin{small}
\begin{verbatim}
...
O pacote \texttt{\{geometry\}}\index{geometry@\texttt{\small{geometry}}} proporciona um meio de configurar a disposição da página. Por exemplo, esta página está com a margem de 4,5cm e sem o espaço do cabeçalho (veja o código-fonte deste tutorial).
\begin{small}
\begin{verbatim}
...
\index{newgeometry@\texttt{\small\textbackslash newgeometry}}
\index{restoregeometry@\texttt{\small\textbackslash restoregeometry}}
\begin{small}
\begin{verbatim}
...
O comando \texttt{\textbackslash pagebreak}\index{pagebreak@\texttt{\small\textbackslash pagebreak}} faz com que os parágrafos se desloquem para preencher toda página, para não deixar espaço vazio no fim. Diferentemente, o comando \texttt{\textbackslash newpage}\index{newpage@\texttt{\small\textbackslash newpage}} não estica os espaços entre os parágrafos, deixando um grande espaço vazio no fim da página. O comando \texttt{\textbackslash clearpage}\index{clearpage@\texttt{\small\textbackslash clearpage}} é similar ao \texttt{\textbackslash newpage}, apenas agindo também nas figuras.

\subsection*{Mesma página}
\addcontentsline{toc}{subsection}{Mesma página}

Caso tenha um conteúdo que queira manter-se em uma mesma página, sem quebra pelo meio, use o ambiente \texttt{\{samepage\}}\index{samepage@\texttt{\small{samepage}}}.
\begin{small}
\begin{verbatim}
   \begin{samepage}
   ...
...
\index{hspace@\texttt{\small\textbackslash hspace}}
\index{vspace@\texttt{\small\textbackslash vspace}}
\begin{small}
\begin{verbatim}
   \hspace{medida}
   \vspace{medida}
...
Uma interessante utilidade, caso queira posicionar os primeiros parágrafos no topo da página e os últimos parágrafos no fim da página, use um destes dois comandos equivalentes para esticar o espaço vazio entre eles:\index{vfill@\texttt{\small\textbackslash vfill}}
\begin{small}
\begin{verbatim}
   \vspace{\fill}
...
\index{thispagestyle@\texttt{\small\textbackslash thispagestyle}}
\index{pagestyle@\texttt{\small\textbackslash pagestyle}}
\begin{small}
\begin{verbatim}
   \thispagestyle{empty}
...
O estilo empty\index{empty@\texttt{\small{empty}}} limpa tanto o cabeçalho quanto o rodapé. No estilo plain\index{plain@\texttt{\small{plain}}}, que é o padrão, o cabeçalho fica vazio e o rodapé contém o número da página no centro.

\section*{Colorindo}
\addcontentsline{toc}{section}{Colorindo}

Uma ou mais páginas podem ser coloridas com o comando \texttt{\textbackslash pagecolor\{\}}\index{pagecolor@\texttt{\small\textbackslash pagecolor}}. Este comando terá efeito da página atual em diante. Por exemplo:
\begin{small}
\begin{verbatim}
...
\index{nopagecolor@\texttt{\small\textbackslash nopagecolor}}
\begin{small}
\begin{verbatim}
   \nopagecolor
\end{verbatim}
...
\index{part@\texttt{\small\textbackslash part}}
\index{chapter@\texttt{\small\textbackslash chapter}}
\index{section@\texttt{\small\textbackslash section}}
\index{subsection@\texttt{\small\textbackslash subsection}}
\index{subsubsection@\texttt{\small\textbackslash subsubsection}}
\index{paragraph@\texttt{\small\textbackslash paragraph}}
\index{subparagraph@\texttt{\small\textbackslash subparagraph}}
...
Em documentos da classe \texttt{book}\index{book@\texttt{\small{book}}}, opcionalmente pode-se seccionar o conteúdo em quatro partes: frontal, principal, apêndice e traseira.
...
A parte em \texttt{\textbackslash frontmatter}\index{frontmatter@\texttt{\small\textbackslash frontmatter}} terá a numeração romana nas páginas e não terá os capítulos numerados. A parte em \texttt{\textbackslash mainmatter}\index{mainmatter@\texttt{\small\textbackslash mainmatter}} terá o comportamento padrão do documento e a sequencia numérica das páginas é reiniciada. A parte \texttt{\textbackslash appendix}\index{appendix@\texttt{\small\textbackslash appendix}} reinicia a numeração de capítulos, usa letras na numeração de capítulos e continua seguindo a numeração das páginas principais. A parte \texttt{\textbackslash backmatter}\index{backmatter@\texttt{\small\textbackslash backmatter}} continua seguindo a numeração das páginas principais mas volta a desativar a numeração dos capítulos.
...
Ou usar o comando \texttt{\textbackslash par}\index{par@\texttt{\small\textbackslash par}} no final da linha; \par Para quebrá-la em um novo parágrafo.

Isto é um texto em uma linha, \newline o comando \texttt{\textbackslash newline}\index{newline@\texttt{\small\textbackslash newline}} ou \texttt{\textbackslash\textbackslash}\index{\texttt{\small{\textbackslash \textbackslash}}} (duas barras invertidas) faz uma quebra de linha, sem criar um novo parágrafo.
...
\index{setlength@\texttt{\small\textbackslash setlength}}
\index{parskip@\texttt{\small\textbackslash parskip}}
\begin{small}
\begin{verbatim}
...
\index{onehalfspacing@\texttt{\small\textbackslash onehalfspacing}}
\index{doublespacing@\texttt{\small\textbackslash doublespacing}}
\index{singlespacing@\texttt{\small\textbackslash singlespacing}}
\index{baselinestretch@\texttt{\small\textbackslash baselinestretch}}
\index{normalsize@\texttt{\small\textbackslash normalsize}}
\begin{small}
\begin{verbatim}
   \onehalfspacing
   \doublespacing
...
\index{\texttt{\small\textbackslash ;}}
\index{\texttt{\small{\textbackslash "!}}}
\index{thinspace@\texttt{\small\textbackslash thinspace}}
\index{medspace@\texttt{\small\textbackslash medspace}}
\index{thickspace@\texttt{\small\textbackslash thickspace}}
\index{negthinspace@\texttt{\small\textbackslash negthinspace}}
\index{negmedspace@\texttt{\small\textbackslash negmedspace}}
\index{negthickspace@\texttt{\small\textbackslash negthickspace}}
\index{quad@\texttt{\small\textbackslash quad}}
\index{qquad@\texttt{\small\textbackslash qquad}}
...
Obs.: O espaço comum é um espaço expansível, isto é, sua largura é flexível para esticar ou contrair, quando o parágrafo possui o alinhamento justificado. Os comandos acima produzem um espaço não expansível, com largura fixa. Como alternativa, o comando \texttt{\textbackslash space}\index{space@\texttt{\small\textbackslash space}} produz um espaço que é expansível.

\subsection*{Indentação}
\addcontentsline{toc}{subsection}{Indentação}

Os parágrafos costumam indentar-se automaticamente. Mas, em uma instalação padrão do \LaTeX, o primeiro não se indenta, somente do segundo em diante. Neste documento, se não tivesse instalado o pacote \texttt{\{indentfirst\}}\index{indentfirst@\texttt{\small{indentfirst}}}, este primeiro parágrafo não estaria indentado.

Para definir um tamanho de indentação, usa-se: \texttt{\textbackslash setlength\{\textbackslash parindent\}\{3em\}}\index{setlength@\texttt{\small\textbackslash setlength}}\index{parindent@\texttt{\small\textbackslash parindent}}
...
\noindent
Já este parágrafo não está indentado, pois antes dele há o comando \texttt{\textbackslash noindent}\index{noindent@\texttt{\small\textbackslash noindent}}

\setlength{\parindent}{1.5em}
Voltando à indentação normal, com: \texttt{\textbackslash setlength\{\textbackslash parindent\}\{1.5em\}}

\indent O comando \texttt{\textbackslash indent}\index{indent@\texttt{\small\textbackslash indent}} força uma indentação, caso não ocorra, mas só se \texttt{\textbackslash parindent} estiver diferente de zero.
...
\index{center@\texttt{\small{center}}}
\index{flushleft@\texttt{\small{flushleft}}}
\index{flushright@\texttt{\small{flushright}}}
...
Existem alguns tamanhos para os caracteres da fonte (e incrementado pelo pacote \texttt{\{moresize\}})\index{moresize@\texttt{\small{moresize}}}, a partir do normal definido entre 10pt, 11pt ou 12pt, os quais são ativados por comandos com sintaxe inline ou para bloco de ambiente:

\index{tiny@\texttt{\small\textbackslash tiny}}
\index{ssmall@\texttt{\small\textbackslash ssmall}}
\index{scriptsize@\texttt{\small\textbackslash scriptsize}}
\index{footnotesize@\texttt{\small\textbackslash footnotesize}}
\index{small@\texttt{\small\textbackslash small}}
\index{normalsize@\texttt{\small\textbackslash normalsize}}
\index{large@\texttt{\small\textbackslash large}}
\index{Large@\texttt{\small\textbackslash Large}}
\index{LARGE@\texttt{\small\textbackslash LARGE}}
\index{huge@\texttt{\small\textbackslash huge}}
\index{Huge@\texttt{\small\textbackslash Huge}}
\index{HUGE@\texttt{\small\textbackslash HUGE}}
...
\index{textbf@\texttt{\small\textbackslash textbf}}
\index{textmd@\texttt{\small\textbackslash textmd}}
\index{textit@\texttt{\small\textbackslash textit}}
\index{textsl@\texttt{\small\textbackslash textsl}}
\index{underline@\texttt{\small\textbackslash underline}}
\index{textsuperscript@\texttt{\small\textbackslash textsuperscript}}
\index{textsubscript@\texttt{\small\textbackslash textsubscript}}
\index{text@\texttt{\small\textbackslash text}}
\index{textnormal@\texttt{\small\textbackslash textnormal}}
\index{textup@\texttt{\small\textbackslash textup}}
\index{textsc@\texttt{\small\textbackslash textsc}}
\index{bfseries@\texttt{\small\textbackslash bfseries}}
\index{mdseries@\texttt{\small\textbackslash mdseries}}
\index{itshape@\texttt{\small\textbackslash itshape}}
\index{normalfont@\texttt{\small\textbackslash normalfont}}
\index{upshape@\texttt{\small\textbackslash upshape}}
\index{scshape@\texttt{\small\textbackslash scshape}}
...
\index{textrm@\texttt{\small\textbackslash textrm}}
\index{texttt@\texttt{\small\textbackslash texttt}}
\index{textsf@\texttt{\small\textbackslash textsf}}
\index{rmfamily@\texttt{\small\textbackslash rmfamily}}
\index{ttfamily@\texttt{\small\textbackslash ttfamily}}
\index{sffamily@\texttt{\small\textbackslash sffamily}}
...
Para colorir um texto, usa-se o comando \texttt{\textbackslash textcolor\{\textit{cor}\}\{...\}}\index{textcolor@\texttt{\small\textbackslash textcolor}} ou \texttt{\{\textbackslash color\{\textit{cor}\} ...\}}\index{color@\texttt{\small\textbackslash color}}. Ou, pode-se definir uma cor diretamente no comando. Exemplos:
...
Para o fundo do texto, usa-se o comando \texttt{\textbackslash colorbox\{\textit{cor}\}\{...\}}\index{colorbox@\texttt{\small\textbackslash colorbox}}:
...
Uma tabela pode ser construída com o ambiente \texttt{\{tabular\}}\index{tabular@\texttt{\small{tabular}}} ou com o ambiente \texttt{\{tabbing\}}\index{tabbing@\texttt{\small{tabbing}}}. O ambiente \texttt{\{tabular\}} requer um argumento que indica quantas colunas terá e qual o alinhamento de cada coluna. Já no ambiente \texttt{\{tabbing\}}, as colunas assim como suas larguras são definidas diretamente pelos separadores.

No ambiente \texttt{\{tabular\}}, para indicar a quantidade de colunas e o respectivo alinhamento, use a quantidade de letras \texttt{l} (alinhamento à esquerda), \texttt{c} (alinhamento ao centro) e \texttt{r} (alinhamento à direita). No conteúdo, as colunas são delimitadas pelo caractere \texttt{\&}. Já no ambiente \texttt{\{tabbing\}}, os separadores das colunas serão os comandos \texttt{\textbackslash =}\index{\texttt{\small\textbackslash =}} ou \texttt{\textbackslash >}\index{\texttt{\small\textbackslash >}} etc. (veja lista). Em ambos os ambientes, ao final de cada linha, use uma quebra.
...
\index{kill@\texttt{\small\textbackslash kill}}
...
\index{multicolumn@\texttt{\small\textbackslash multicolumn}}
...
\index{multirow@\texttt{\small\textbackslash multirow}}
...
\newpage
Também é possível colorir as tabelas, colorindo linhas com o comando \texttt{\textbackslash rowcolor\{\}}, colorindo colunas com o comando \texttt{\textbackslash columncolor\{\}}\index{columncolor@\texttt{\small\textbackslash columncolor}} e colorindo células com o comando \texttt{\textbackslash cellcolor\{\}}:\index{cellcolor@\texttt{\small\textbackslash cellcolor}}
...
Uma especificação por toda coluna pode ser aplicada no argumento do ambiente \texttt{\{tabular\}}, usando \texttt{>\{\textbackslash comando\}}\index{\texttt{\small{<\{\}}}} para comandos executados antes de cada elemento da coluna e \texttt{<\{\textbackslash comando\}}\index{\texttt{\small{>\{\}}}} para comandos que serão executados após cada elemento da coluna, como foi usado acima com o comando \texttt{\textbackslash columncolor\{\}}.

\subsection*{Lista}
\addcontentsline{toc}{subsection}{Lista}

As listas são construídas com o ambiente \texttt{\{itemize\}}\index{itemize@\texttt{\small{itemize}}}, para listas não-ordenadas, ou com o ambiente \texttt{\{enumerate\}}\index{enumerate@\texttt{\small{enumerate}}}, para listas ordenadas:
\index{item@\texttt{\small\textbackslash item}}
\begin{multicols}{2}
\begin{verbatim}
\begin{itemize}
    \item Arroz;
...
\index{inparaenum@\texttt{\small{inparaenum}}}
...
Para inserir uma nota de rodapé\footnote{Uma anotação colocada ao pé de uma página.\label{notarodape}}, use o comando \texttt{\textbackslash footnote\{\textit{nota}\}}\index{footnote@\texttt{\small\textbackslash footnote}} logo após a palavra que será comentada.
...
\index{multicols@\texttt{\small\textbackslash multicols}}
\index{vwcol@\texttt{\small\textbackslash vwcol}}
\index{minipage@\texttt{\small\textbackslash minipage}}
\index{tabular@\texttt{\small\textbackslash tabular}}
...
Veja alguns exemplos. Nos ambientes de texto fluido foi usado o comando \texttt{\textbackslash columnbreak}\index{columnbreak@\texttt{\small\textbackslash columnbreak}} ou o comando \texttt{\textbackslash newpage}\index{newpage@\texttt{\small\textbackslash newpage}} para forçar a quebra de coluna:

\begin{small}\index{multicols@\texttt{\small\textbackslash multicols}}
\begin{verbatim}
   \begin{multicols}{2}
      \lipsum[66]\par
      \columnbreak
...
\begin{small}\index{minipage@\texttt{\small\textbackslash minipage}}
\begin{verbatim}
...
\newpage
\begin{small}\index{tabular@\texttt{\small\textbackslash tabular}}\index{parbox@\texttt{\small\textbackslash parbox}}
\begin{verbatim}
...
\begin{small}\index{vwcol@\texttt{\small\textbackslash vwcol}}
\begin{verbatim}
...
No \LaTeX, a página do sumário é simplesmente criada automaticamente pelo comando \texttt{\textbackslash tableofcontents}\index{tableofcontents@\texttt{\small\textbackslash tableofcontents}}, colocado no início do ambiente \texttt{\{document\}}, logo após a página do título e da folha de rosto, se houver, e dentro parte marcada com \texttt{\textbackslash frontmatter}, se também houver este comando.

Caso não esteja usando um pacote de localização, é possível alterar o nome do sumário redefinindo o comando \texttt{\textbackslash contentsname}\index{contentsname@\texttt{\small\textbackslash contentsname}}, assim:
...
Caso não queira adicionar os capítulos, seções e subseções automaticamente, adicione o caractere \texttt{*} nos respectivos comandos. E para incluir manualmente estes capítulos, seções e subseções no sumário, use, após os comandos deles, o comando \texttt{\textbackslash addcontentsline}\index{addcontentsline@\texttt{\small\textbackslash addcontentsline}}. Exemplos para algumas divisões:
\begin{small}
\begin{verbatim}
...
No \LaTeX, o índice pode ser criado com os comandos do pacote \texttt{\{imakeidx\}}\index{imakeidx@\texttt{\small{imakeidx}}}, então, coloque no preâmbulo o comando: \texttt{\small\textbackslash usepackage\{imakeidx\}}

Ainda no preâmbulo, coloque o comando abaixo para disparar a criação do índice. Estes parâmetros são autoexplicativos:\index{makeindex@\texttt{\small\textbackslash makeindex}}

\texttt{\small\textbackslash makeindex[columns=3, title=Índice, intoc]}

Para cada termo no documento, que desejar incluir no índice, use próximo ao termo o comando \texttt{\textbackslash index\{termo\}}\index{index@\texttt{\small\textbackslash index}}. É possível incluir um apelido alternativo, específico para o critério de ordenação, usando a sintaxe \texttt{\{apelido@termo\}}.

Por fim, para criar a página do índice, coloque na parte traseira do documento o comando \texttt{\small\textbackslash printindex}\index{printindex@\texttt{\small\textbackslash printindex}}.
...
\index{label@\texttt{\small\textbackslash label}}
\index{ref@\texttt{\small\textbackslash ref}}
\index{pageref@\texttt{\small\textbackslash pageref}}
...
O uso do pacote \texttt{\{hyperref\}}\index{hyperref@\texttt{\small{hyperref}}} transforma automaticamente todas as referências internas em ligações. Mas é possível adicionar trechos de textos que serão ligações para as marcações existentes, independente das referências.

Aproveitando o rótulo da marcação (\texttt{\textbackslash label\{marcador\}})\index{label@\texttt{\small\textbackslash label}}, para criar uma ligação interna, usa-se o comando \texttt{\textbackslash hyperref[marcador]\{texto\}}.\index{hyperref@\texttt{\small\textbackslash hyperref}}
...
O pacote \texttt{\{hyperref\}} possui uma extensa possibilidade de configuração. Veja o código-fonte deste tutorial para ver o comando \texttt{\textbackslash hypersetup\{\}}\index{hypersetup@\texttt{\small\textbackslash hypersetup}}, logo no preâmbulo. Este pacote também formata muitas coisas para o PDF exportado.

\section*{Ligações web}
\addcontentsline{toc}{section}{Ligações web}

As ligações para páginas Web são criadas com os comandos \texttt{\textbackslash href\{\}}\index{href@\texttt{\small\textbackslash href}} ou \texttt{\textbackslash url\{\}}\index{url@\texttt{\small\textbackslash url}}. a diferença entre eles é que \texttt{\textbackslash href} permite uma legenda para o endereço, enquanto o \texttt{\textbackslash url} imprime diretamente o endereço. Segue os exemplos:
...
Com os comandos nativos, provenientes desde o \TeX, pode-se obter os registros do ano, mês, dia e hora atual, usando \texttt{\textbackslash year}\index{year@\texttt{\small\textbackslash year}}, \texttt{\textbackslash month}\index{month@\texttt{\small\textbackslash month}}, \texttt{\textbackslash day}\index{day@\texttt{\small\textbackslash day}} e \texttt{\textbackslash time}\index{time@\texttt{\small\textbackslash time}} (este retorna os minutos desde a zero hora). Estes registros contém os valores coletados no momento do processamento do código-fonte .tex. Contudo, estes valores não são acessíveis com o uso direto destes comandos, sendo necessário utilizar em conjunto com outro comando nativo, que pode ser \texttt{\textbackslash the}\index{the@\texttt{\small\textbackslash the}} ou \texttt{\textbackslash number}.\index{number@\texttt{\small\textbackslash number}} 
...
\index{DTMdisplaydate@\texttt{\small\textbackslash DTMdisplaydate}}
\index{DTMdate@\texttt{\small\textbackslash DTMdate}}
...
\index{DTMtoday@\texttt{\small\textbackslash DTMtoday}}

\texttt{\small\textbackslash DTMtoday}

Que produz algo como: \DTMtoday

Obs.: Existe o comando \texttt{\textbackslash today}\index{today@\texttt{\small\textbackslash today}}, suportado pelo pacote \texttt{\{datetime2\}}, porém é usado também por outros pacotes e então, para evitar imprevistos, prefira usar o \texttt{\textbackslash DTMtoday}, que é exclusivo.

Para imprimir o nome de um mês, na língua portuguesa, use o comando:
\index{DTMportugesmonthname@\texttt{\small\textbackslash DTMportugesmonthname}}
...
\index{DTMportugesweekdayname@\texttt{\small\textbackslash DTMportugesweekdayname}}
\index{DTMportugesWeekdayname@\texttt{\small\textbackslash DTMportugesWeekdayname}}
...
\index{DTMdisplaytime@\texttt{\small\textbackslash DTMdisplaytime}}
\index{DTMtime@\texttt{\small\textbackslash DTMtime}}
...
\index{DTMcurrenttime@\texttt{\small\textbackslash DTMcurrenttime}}
...
\index{DTMdisplay@\texttt{\small\textbackslash DTMdisplay}}
...
\index{DTMnow@\texttt{\small\textbackslash DTMnow}}
...
Ainda, existem comandos para armazenar uma data ou uma hora, com um nome escolhido e assim tornar possível o uso posterior no documento. Por exemplo, o comando \texttt{\textbackslash DTMsavedate\{minhadata\}\{2016-02-10\}}\index{DTMsavedate@\texttt{\small\textbackslash DTMsavedate}} armazena a data especificada em \texttt{minhadata} e o comando \texttt{\textbackslash DTMusedate\{minhadata\}}\index{DTMusedate@\texttt{\small\textbackslash DTMusedate}} imprime a data armazenada em \texttt{minhadata}.

A mesma coisa para a hora. Por exemplo, o comando \texttt{\textbackslash DTMsavetime\{minhahora\} \{10:00:00\}}\index{DTMsavetime@\texttt{\small\textbackslash DTMsavetime}} armazena a hora especificada em \texttt{minhahora} e o comando \texttt{\textbackslash DTMusetime \{minhahora\}}\index{DTMusetime@\texttt{\small\textbackslash DTMusetime}} imprime a hora armazenada em \texttt{minhahora}.

Para extrair os valores isoladamente, de uma data ou hora armazenada, existem os comandos: \texttt{\textbackslash DTMfetchyear\{nome\}}\index{DTMfetchyear@\texttt{\small\textbackslash DTMfetchyear}}, \texttt{\textbackslash DTMfetchmonth\{nome\}}\index{DTMfetchmonth@\texttt{\small\textbackslash DTMfetchmonth}}, \texttt{\textbackslash DTMfetchday\{nome\}}\index{DTMfetchday@\texttt{\small\textbackslash DTMfetchday}}, \texttt{\textbackslash DTMfetchhour\{nome\}}\index{DTMfetchhour@\texttt{\small\textbackslash DTMfetchhour}}, \texttt{\textbackslash DTMfetchminute\{nome\}}\index{DTMfetchminute@\texttt{\small\textbackslash DTMfetchminute}} e \texttt{\textbackslash DTMfetchsecond\{nome\}}\index{DTMfetchsecond@\texttt{\small\textbackslash DTMfetchsecond}}.
...
\index{DTMsetdatestyle@\texttt{\small\textbackslash DTMsetdatestyle}}
...
\index{DTMsetup@\texttt{\small\textbackslash DTMsetup}}
\index{DTMtryregional@\texttt{\small\textbackslash DTMtryregional}}
\begin{small}
\begin{verbatim}
...
O formato da hora possui um estilo disponível, com o comando \texttt{\textbackslash DTMsettimestyle \{hmmss\}}\index{DTMsettimestyle@\texttt{\small\textbackslash DTMsettimestyle}} a hora impressa será no formato: \DTMtime{5:7:20}
...
Para imprimir uma expressão matemática no parágrafo, na mesma linha, pode-se usar os delimitadores de ambiente \texttt{\$} e \texttt{\$}\index{\texttt{\small\$}}, \texttt{\textbackslash (}\index{\texttt{\small\textbackslash (}} e \texttt{\textbackslash )}\index{\texttt{\small\textbackslash )}} ou \texttt{\textbackslash begin\{math\}} e \texttt{\textbackslash end\{math\}}\index{math@\texttt{\small{math}}}. Por exemplo, \( ax^2 + bx + c = 0 \)  pode ser impresso digitando \texttt{\textbackslash ( ax\textasciicircum2 + bx + c = 0 \textbackslash )}
...
Para imprimir uma expressão em uma nova linha, chamado modo de exibição, usa-se os delimitadores \texttt{\textbackslash [}\index{\texttt{\small\textbackslash [}} e \texttt{\textbackslash ]}\index{\texttt{\small\textbackslash ]}} ou \texttt{\textbackslash begin\{displaymath\}} e \texttt{\textbackslash end\{displaymath\}}\index{displaymath@\texttt{\small{displaymath}}}. Por exemplo, \texttt{\textbackslash [ \textbackslash frac\{9\}\{12\} + \textbackslash frac\{5\}\{34\} + \textbackslash frac\{7\}\{68\} = 1 \textbackslash ]} produz:

\[\frac{9}{12} + \frac{5}{34} + \frac{7}{68} = 1\]

Uma outra forma de imprimir uma expressão matemática no modo de exibição, inclusive alinhá-la, numerá-la e indexá-la no documento, é com os ambientes \texttt{\{equation\}}\index{equation@\texttt{\small{equation}}} ou \texttt{\{align\}}\index{align@\texttt{\small{align}}}. Exemplos:
\begin{small}
\begin{verbatim}
   \begin{align}
...
\begin{small}\index{aligned@\texttt{\small{aligned}}}
\begin{verbatim}
   \begin{equation*}
      \begin{aligned}[c]
...
Obs.: O caractere \texttt{\&}\index{\texttt{\small\&}} marca a posição de alinhamento. Nestas sequencias de equações, foi adotada a posição no sinal de igual. O asterisco no comando cancela a numeração da equação no documento.
...
\index{\texttt{\small\textbackslash \_}}
\index{textasciicircum@\texttt{\small\textbackslash textasciicircum}}
\index{sqrt@\texttt{\small\textbackslash sqrt}}
\index{frac@\texttt{\small\textbackslash frac}}
\index{sum@\texttt{\small\textbackslash sum}}
\index{limits@\texttt{\small\textbackslash limits}}
\index{infty@\texttt{\small\textbackslash infty}}
\index{int@\texttt{\small\textbackslash int}}
\begin{center}
...
Com o uso do pacote \texttt{\{graphicx\}}\index{graphicx@\texttt{\small{graphicx}}} é fácil inserir imagens no documento em \LaTeX. Além de inserir, é possível posicionar, redimensionar e rotacionar.

\subsection*{Caminho}
\addcontentsline{toc}{subsection}{Caminho}

Antes, pode ser útil definir previamente o caminho das imagens que irão compor o documento. O comando \texttt{\textbackslash graphicspath\{\}} pode ser declarado no preâmbulo.\index{graphicspath@\texttt{\small\textbackslash graphicspath}}
...
O carregamento da imagem é pelo comando \texttt{\textbackslash includegraphics[]\{\}}\index{includegraphics@\texttt{\small\textbackslash includegraphics}}. Se a imagem estiver no caminho, basta informar o nome do arquivo, por exemplo:
...
O carregamento da imagem torna-se mais preciso se estiver no ambiente \texttt{\{figure\}}\index{figure@\texttt{\small{figure}}}. Com este ambiente podemos especificar o parâmetro do posicionamento:
\begin{small}
\begin{verbatim}
   \begin{figure}[h!]
      \centering
...
\newpage
Adicionando o pacote \texttt{\{wrapfig\}}\index{wrapfig@\texttt{\small{wrapfig}}}, o texto ganha a possibilidade de envolver a imagem carregada. Para isso usa-se o ambiente \texttt{\{wrapfigure\}}\index{wrapfigure@\texttt{\small{wrapfigure}}}. Segue uma ilustração:
\begin{small}
\begin{verbatim}
...
Baseado no pacote PGF/TikZ existe o pacote \texttt{\{pgfplots\}}\index{pgfplots@\texttt{\small{pgfplots}}}, para construir uma plotagem de dados, de funções etc. Conta com um segundo componente, o pacote \texttt{\{pgfplotstable\}}\index{pgfplotstable@\texttt{\small{pgfplotstable}}}, que faz a formatação e o processamento de tabelas numéricas. Os ambientes de plotagem do \texttt{\{pgfplots\}} dependem do ambiente \texttt{\{tikzpicture\}}.\index{tikzpicture@\texttt{\small{tikzpicture}}}
...
Diversos ambientes irão estruturar uma plotagem, começando pelo ambiente gráfico \texttt{\{tikzpicture\}}\index{tikzpicture@\texttt{\small{tikzpicture}}}, que forma a imagem. Interiormente, tem-se os ambientes específicos da plotagem, em relação aos eixos de escala normal ou escala logarítmica, com os ambientes \texttt{\{axis\}}\index{axis@\texttt{\small{axis}}}, \texttt{\{semilogaxis\}}\index{semilogaxis@\texttt{\small{semilogaxis}}} ou \texttt{\{loglogaxis\}}\index{loglogaxis@\texttt{\small{loglogaxis}}}. Sintaxe básica:
\begin{small}
\begin{verbatim}
   \begin{tikzpicture}
      \begin{axis}[ ]
...
Dentro do ambiente do eixo insere-se o comando para adicionar uma plotagem, com \texttt{\textbackslash addplot}\index{addplot@\texttt{\small\textbackslash addplot}}, ou \texttt{\textbackslash addplot3}\index{addplot3@\texttt{\small\textbackslash addplot3}} para visualização em 3D. Neste comando, especifica-se a origem dos dados, se será por uma função, por coordenadas ou fornecidos por uma tabela em um arquivo externo. Também pode receber um argumento com diversos parâmetros:\index{coordinates@\texttt{\small{coordinates}}}\index{table@\texttt{\small{table}}}
\begin{small}
\begin{verbatim}
   \addplot[
      blue,
...
\index{grave@\texttt{\small\textbackslash grave}}
\index{acute@\texttt{\small\textbackslash acute}}
\index{hat@\texttt{\small\textbackslash hat}}
\index{widehat@\texttt{\small\textbackslash widehat}}
\index{tilde@\texttt{\small\textbackslash tilde}}
\index{widetilde@\texttt{\small\textbackslash widetilde}}
\index{mathring@\texttt{\small\textbackslash mathring}}
\index{bar@\texttt{\small\textbackslash bar}}
\index{dot@\texttt{\small\textbackslash dot}}
\index{ddot@\texttt{\small\textbackslash ddot}}
\index{breve@\texttt{\small\textbackslash breve}}
\index{check@\texttt{\small\textbackslash check}}
\index{vec@\texttt{\small\textbackslash vec}}
...
\index{Biggl@\texttt{\small\textbackslash Biggl}}
\index{biggl@\texttt{\small\textbackslash biggl}}
\index{Bigl@\texttt{\small\textbackslash Bigl}}
\index{bigl@\texttt{\small\textbackslash bigl}}
\index{bigr@\texttt{\small\textbackslash bigr}}
\index{Bigr@\texttt{\small\textbackslash Bigr}}
\index{biggr@\texttt{\small\textbackslash biggr}}
\index{Biggr@\texttt{\small\textbackslash Biggr}}
...
\addcontentsline{toc}{subsection}{Nativos do \LaTeX}\index{latexsym@\texttt{\small{latexsym}}}
...
\addcontentsline{toc}{subsection}{Text Companion}\index{textcomp@\texttt{\small{textcomp}}}
...
\addcontentsline{toc}{subsection}{American Mathematical Society}\index{amsmath@\texttt{\small{amsmath}}}\index{amssymb@\texttt{\small{amssymb}}}
...
\addcontentsline{toc}{subsection}{Font Awesome}\index{fontawesome@\texttt{\small{fontawesome}}}
...
\addcontentsline{toc}{subsection}{Zapf Dingbats}\index{pifont@\texttt{\small{pifont}}}
...
Dica: Este pacote \texttt{\{pifont\}} traz comandos interessantes. Dois comandos de ambiente \texttt{\{dinglist\}}\index{dinglist@\texttt{\small{dinglist}}} e \texttt{\{dingautolist\}}\index{dingautolist@\texttt{\small{dingautolist}}} que constrõem listas rotuladas com os próprios caracteres da fonte. E dois comandos para preenchimento linear \texttt{\textbackslash dingfill}\index{dingfill@\texttt{\small\textbackslash dingfill}} e \texttt{\textbackslash dingline}\index{dingline@\texttt{\small\textbackslash dingline}}. Veja exemplos:
